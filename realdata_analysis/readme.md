...
#for(ig in 1:10){
  ...
  tempa=tryCatch({cal_w2_pval(count_per_gene=cur_sim,meta_individual=cur_ind,meta_phenotype=cur_pheno,perm_num=200,unif_round_unit=0.5)}, error = function(e) {NA} )
  op_pval[ig]=tryCatch({tempa[[1]]}, error = function(e) {NA} )
  bc_case_ob[[ig]]=tryCatch({tempa[[2]]}, error = function(e) {NA} )
  bc_ctrl_ob[[ig]]=tryCatch({tempa[[3]]}, error = function(e) {NA} )
  if(!(is.na(op_pval[ig])) && op_pval[ig]<2/200){
    op_pval[ig]=tryCatch({cal_w2_pval(count_per_gene=cur_sim,meta_individual=cur_ind,meta_phenotype=cur_pheno,perm_num=1000,unif_round_unit=0.5)[[1]]}, error = function(e) {NA} )}
  if(!(is.na(op_pval[ig])) && op_pval[ig]<2/1000){
    op_pval[ig]=tryCatch({cal_w2_pval(count_per_gene=cur_sim,meta_individual=cur_ind,meta_phenotype=cur_pheno,perm_num=5000,unif_round_unit=0.5)[[1]]}, error = function(e) {NA} )}
  if(!(is.na(op_pval[ig])) && op_pval[ig]<2/5000){
    op_pval[ig]=tryCatch({cal_w2_pval(count_per_gene=cur_sim,meta_individual=cur_ind,meta_phenotype=cur_pheno,perm_num=20000,unif_round_unit=0.5)[[1]]}, error = function(e) {NA} )}
  if(!(is.na(op_pval[ig])) && op_pval[ig]<2/20000){
    op_pval[ig]=tryCatch({cal_w2_pval(count_per_gene=cur_sim,meta_individual=cur_ind,meta_phenotype=cur_pheno,perm_num=100000,unif_round_unit=0.5)[[1]]}, error = function(e) {NA} )}
  if(!(is.na(op_pval[ig])) && op_pval[ig]<2/100000){
    op_pval[ig]=tryCatch({cal_w2_pval(count_per_gene=cur_sim,meta_individual=cur_ind,meta_phenotype=cur_pheno,perm_num=500000,unif_round_unit=0.5)[[1]]}, error = function(e) {NA} )}
...
While we can run our compared method with

## MAST
```{r run_MAST,eval=FALSE}
cell_id=meta$cell
gene_id=dimnames(logcount_data)[[1]]
rownames(logcount_data)=gene_id
colnames(logcount_data)=cell_id
...
colData(sca)$cngeneson = apply(logcount_data>0,2,mean)
...
## aggregateBioVar and DESeq2
```{r run_aggregateBioVar,eval=FALSE}
library(aggregateBioVar)
library(SummarizedExperiment, quietly = TRUE)
library(SingleCellExperiment, quietly = TRUE)
library(DESeq2, quietly = TRUE)

sca=SingleCellExperiment(list(counts=rawcount_data), 
                         colData=data.frame(wellKey=cell_id), 
                         rowData=data.frame(primerid=gene_id))
colData(sca)$diagnosis =as.factor(meta$diagnosis)
colData(sca)$ind = as.factor(meta$individual)
colData(sca)$cluster = as.factor(meta$cluster)

colData(sca)

sca_argBV =aggregateBioVar(scExp = sca,subjectVar = "ind", cellVar = "cluster")

#cluster 28 doesn't work
for(cluster_tag in 29:length(total_cluster)){
  agrBV_pval0=NA
  agrBV_pval1=NA
  cell_dds=NA
  subj_dds=NA
  #agreBV_pval0: do it with teh version of direct DEseq2.
  sca_cur =sca[, which(as.character(sca$cluster) == total_cluster[cluster_tag])]
  sca_cur$diagnosis = as.factor(sca_cur$diagnosis)
  
  cell_dds =DESeqDataSetFromMatrix(
      countData = assay(sca_cur, "counts"),
      colData = colData(sca_cur),
      design = ~ diagnosis
    )
  cell_dds = DESeq(cell_dds)
  agrBV_pval0 = results(cell_dds)$pvalue
  
  #agreBV_pval1: do aggregate and then DEseq2.
  subj_dds =DESeqDataSetFromMatrix(
      countData = assay(sca_argBV[[(cluster_tag + 1)]], "counts"),
      colData = colData(sca_argBV[[(cluster_tag + 1)]]),
      design = ~ diagnosis
    )
  subj_dds = DESeq(subj_dds)
  agrBV_pval1 = results(subj_dds)$pvalue
  
  names(agrBV_pval0)=gene_id #DESeq2
  names(agrBV_pval1)=gene_id #aggregateBioVar
  print(paste(cluster_tag,names(sca_argBV)[(cluster_tag + 1)],total_cluster[cluster_tag]))
}

```

## MUSCAT
```{r run_muscat,eval=FALSE}
dim(rawcount_data)
rawcount_data[1:10,1:10]
cell_id=as.character(tmeta$Cell)
gene_id=dimnames(rawcount_data)[[1]]
rownames(rawcount_data)=gene_id
colnames(rawcount_data)=cell_id
...
#note Here the names are different
colData(sca)$cluster_id = as.factor(tmeta$cluster)
colData(sca)$sample_id = as.factor(tmeta$ind)
colData(sca)$group_id=as.factor(tmeta$diagnosis)

n_cells=table(tmeta$ind)
n_cells=as.numeric(n_cells[match(unique(tmeta$ind),names(n_cells))])
group_id=as.factor(tmeta$diagnosis[match(unique(tmeta$ind),tmeta$ind)])
sample_id=as.factor(unique(tmeta$ind))
...
#Sample Level analysis: Pseudobulk method
res1 = pbDS(pb1, verbose = FALSE,min_cells = 10)
tbl1 = res1$table[[1]]

for(cluster_tag in  1:length(tbl1)){
  cur_cluster=match(names(tbl1),total_cluster)[cluster_tag]
  pb_pval1=NA
  pb_pval1=tbl1[[cluster_tag]]$p_val
  names(pb_pval1)=tbl1[[cluster_tag]]$gene
  print(paste(cluster_tag,names(tbl1[cluster_tag]),total_cluster[cur_cluster]))
}
```

## Mann Whitney U
```{r run_ranksum,eval=FALSE}
print("start rk")
...
sca=SingleCellExperiment(list(counts=rawcount_data), 
                         colData=data.frame(wellKey=cell_id), 
                         rowData=data.frame(primerid=gene_id))
colData(sca)$diagnosis =as.factor(tmeta$diagnosis)
colData(sca)$ind = as.factor(tmeta$individual)
colData(sca)$cluster = as.factor(tmeta$cluster)

colData(sca)

sca_rk =aggregateBioVar(scExp = sca,subjectVar = "ind", cellVar = "cluster")

#cluster_tag==28 doesn't work for single DESeq with 3K
for(cluster_tag in 1:length(total_cluster)){
  rk_pval0=NA
  rk_pval1=NA
  #rk_pval0: do it with the version of direct ranksum.
  sca_cur =sca[, which(as.character(sca$cluster) == total_cluster[cluster_tag])]
  sca_cur$diagnosis = as.factor(sca_cur$diagnosis)
  
  rk_pval0=apply(assay(sca_cur, "counts"),1,function(x){wilcox.test(x[sca_cur$diagnosis!="Control"],x[sca_cur$diagnosis=="Control"])$p.value})
  
  #rk_pval1: do aggregate and then ranksum.
  sca_cur_rk=sca_rk[[(cluster_tag + 1)]]
  rk_pval1=apply(assay(sca_cur_rk, "counts"),1,function(x){wilcox.test(x[colData(sca_cur_rk)$diagnosis!="Control"],x[colData(sca_cur_rk)$diagnosis=="Control"])$p.value})
  
  names(rk_pval0)=gene_id
  names(rk_pval1)=gene_id
  print(paste(cluster_tag,names(sca_rk)[(cluster_tag + 1)],total_cluster[cluster_tag]))
}
...
#read_in
op_padj=readRDS(paste0(dir,"IPF/op_padj.rds"))
gene_id=names(op_padj[[1]])
...
pval_num=sapply(op_padj,function(x){sum(x<=0.1,na.rm=TRUE)}) #adjusted FDR <0.1
...
for(i_cluster in c(7,11,2,8,12)){ 
  ...
  sig_index_op=c(order(cur_pval,decreasing = FALSE))[1:sum(cur_padj<=0.1,na.rm = TRUE)]
  
  mt_index=grep("^MT",names(cur_pval)) #remove MT genes
  sig_index_op=setdiff(sig_index_op,mt_index)
  
  ###plot 
  for(i_sig in c(sig_index_op)){
